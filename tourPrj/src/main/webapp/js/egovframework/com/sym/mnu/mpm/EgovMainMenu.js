...
function createTree(arrName, vYeobu, checkValue) {
	...
	if (treeNodes.length > 0) {
		...
		if (startNode !=0) {
			...
			vHtmlCode +="<li class='on'><a href=''>" + nodeValues[2] + "</a></li>"
...
function addTreeNode(parentNode, recursedNodes) {
	for (var i = 0; i < treeNodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			if (hasChildNode) {
				vHtmlCode +=vHtmlCodeBg+nodeValues[2]+"";
			} else{
				// Start link
				if(recursedNodes.length==1){
				   vHtmlCode +=vHtmlCodeBg+"<li class='active'><a href=javascript:fn_MovePage('" + i + "');><i class='fa fa-caret-right' aria-hidden='true'></i> "+nodeValues[2]+"</a></li>";
				}else{
				   vHtmlCode +=vHtmlCodeBgList+"<li class='active'><a href=javascript:fn_MovePage('" + i + "');><i class='fa fa-caret-right fa-ul' aria-hidden='true'></i> "+nodeValues[2]+"</a></li>";
				}
			}
